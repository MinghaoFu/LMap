...
typedef bool LRbTreeNode_color;
const LRbTreeNode_color red = false;
const LRbTreeNode_color black = true;

template <class Value>
struct LRbTreeNode {
    typedef Value value_type;
    typedef LRbTreeNode_color color_type;
    typedef LRbTreeNode* node_ptr;
    ...
    LRbTreeNode(const node_ptr& node) {
...
struct LRbTreeIterator {
    ...
    typedef LRbTreeNode_color color_type;
    typedef LRbTreeNode<value_type>* node_ptr;
    typedef LRbTreeIterator<value_type, reference> iterator;
    typedef LRbTreeIterator<value_type, const_reference> const_iterator;

    LRbTreeIterator() {}
    LRbTreeIterator(node_ptr x) { m_node = x; }
    LRbTreeIterator(const iterator& it) { m_node = it.m_node; }
    LRbTreeIterator(iterator& it) { m_node = it.m_node; }
...
private:
    iterator m_it;
public:
    LRbTreeReverseIterator(iterator x) { m_it = x; }
    LRbTreeReverseIterator(node_ptr x) { m_it = x; }
...
private:
    ...
    typedef const Value& const_reference;
    typedef LRbTreeNode_color color_type;
    typedef LRbTreeNode<value_type> node;
    typedef LRbTreeNode<value_type>* node_ptr;
    typedef LRbTree<Key, Value> tree;
    typedef size_t size_type;
public:
    typedef LRbTreeIterator<value_type, reference> iterator;
    typedef LRbTreeIterator<value_type, const_reference> const_iterator;
    typedef LRbTreeReverseIterator<iterator> reverse_iterator;
    typedef LRbTreeReverseIterator<const_iterator> const_reverse_iterator;
...
public:
    ...
    void LRbTreeRotateLeft(node_ptr x, node_ptr& root) {
    ...
    void LRbTreeRotateRight(node_ptr x, node_ptr& root) {
    ...
    void LRbTreeRebalance(node_ptr x, node_ptr& root) {
        x->m_color = red;
            ...
            if (x->m_pParent == x->m_pParent->m_pParent->m_pLeft) {
                ...
                if (y && y->m_color == red) {
                    ...
                    if (x == x->m_pParent->m_pRight) {
                        x = x->m_pParent;
                        LRbTreeRotateLeft(x, root);
                    }
                    LRbTreeRotateRight(x->m_pParent->m_pParent, root);
                    x->m_pParent->m_color = black;
                ...
                if (y && y->m_color == red) {
                    ...
                    if (x == x->m_pParent->m_pLeft) {
                        x = x->m_pParent;
                        LRbTreeRotateRight(x, root);
                    }
                    LRbTreeRotateLeft(x->m_pParent->m_pParent, root);
                    x->m_pParent->m_color = black;
    ...
    node_ptr LRbTreeRebalanceForErase(node_ptr z,
                                node_ptr& root,
                                node_ptr& leftmost,
                                node_ptr& rightmost) {
        node_ptr y = z;
        ...
        if (y->m_color != red) {
          while (x != root && (x == 0 || x->m_color == black))
              ...
              if (w->m_color == red) {
                ...
                LRbTreeRotateLeft(x_parent, root);
                w = x_parent->m_pRight;
              }
              ...
              if (w->m_color == red) {
                w->m_color = black;
                x_parent->m_color = red;
                LRbTreeRotateRight(x_parent, root);
                w = x_parent->m_pLeft;
              }
              if ((w->m_pRight == 0 || w->m_pRight->m_color == black) &&
    ...
    void erase(iterator position) {
        node_ptr y = LRbTreeRebalanceForErase(position.m_node,
                                                 m_header->m_pParent,
                                                 m_header->m_pLeft,
                                                 m_header->m_pRight);
        destroyNode(y);
    ...
    size_type erase(const key_type& k) {
        ...
        node_ptr y = LRbTreeRebalanceForErase(x,
                                                 m_header->m_pParent,
                                                 m_header->m_pLeft,
                                                 m_header->m_pRight);
        destroyNode(y);
...
public:
    ...
    bool rbVerify() {
        ...
        for (iterator it = begin(); it != end(); ++it) {
          ...
          if (L && L->m_value.key() > x->m_value.key())
            return false;
          if (R && R->m_value.key() < x->m_value.key())
            return false;
