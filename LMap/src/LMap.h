...
private:
    typedef rbTree<key_type, value_type> tree;
    tree m_t;
...
public:
    ...
    int test();
    LMap() : m_t(Compare()) {};
    explicit LMap(const key_compare& comp) : m_t(comp) {}
    template <class InputIterator>
    LMap(InputIterator first, InputIterator last, const Compare& comp = Compare()) : m_t(comp) { m_t.insertUnique(first, last); }
    
    LMap(const LMap<key_type, data_type, key_compare>& x) : m_t(x.m_t) {}
    LMap<key_type, data_type, key_compare>& operator=(const LMap<key_type, data_type, key_compare>& x)
    {
      m_t = x.m_t;
      return *this;
    }
    
    key_compare keyComp() const { return m_t.m_keyCompare; }
    ...
    void swap(LMap<key_type, data_type, key_compare>& x) { m_t.swap(x.t); }
